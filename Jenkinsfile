...
pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            ...
            steps {
                script {
                    //assign userinput to env variable 
                    env.ENV = input message: "Choose environment to deploy to", ok: "Done", parameters: [choice(name: 'FIRST', choices: ['fix', 'dev', 'prod'], description: 'Env to deploy to based on choice parameter')]
                    gv.deployApp()
